...
//DELAY_MS(1000);
 ...
 I2C_WriteReg(0x38>>1,CTRL_REG1,ASLP_RATE_20MS+DATA_RATE_5MS);  //0为+-2g，1为+-4g，2为+-8g
 DELAY_MS(5);
 I2C_WriteReg(0x38>>1,XYZ_DATA_CFG_REG, FULL_SCALE_2G);  // 使能低通滤波
 DELAY_MS(5);
 I2C_WriteReg(0x38>>1,CTRL_REG1 , ACTIVE_MASK); //0x2a为控制寄存器1 激活mma8451
 DELAY_MS(5);
 
 
// I2C_WriteReg(0x38>>1,0x0E, 1);  //0为+-2g，1为+-4g，2为+-8g
// DELAY_MS(5);
// I2C_WriteReg(0x38>>1,0x0F, 0X10);  // 使能低通滤波
// DELAY_MS(5);
// I2C_WriteReg(0x38>>1,0x2A , 1); //0x2a为控制寄存器1 激活mma8451
// DELAY_MS(5);
// 
