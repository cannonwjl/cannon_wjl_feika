...
extern u16 AD3_Vector[10];
extern u16 AD2_Vector[10];

extern u8 MODE_KEY_AD;//电磁模式调试选项
...
void adc_head_init(void)
{
    adc_init(ADC1,AD8);         //电源电压AD初始化//           //PTB0
    adc_init(ADC1,AD9);         //电源电压AD初始化//           //PTB1
    adc_init(ADC0,AD12);        //电源电压AD初始化//           //PTB2
    adc_init(ADC0,AD13);        //电源电压AD初始化//           //PTB3
    adc_init(ADC1,AD14);        //电源电压AD初始化//           //PTB10
    adc_init(ADC1,AD15);        //电源电压AD初始化//          //PTB11
}

void Get_head_ad(int *headAD)
{
headAD[0]=ad_ave (ADC1,AD8  , ADC_12bit,11);
headAD[1]=ad_ave (ADC1,AD9  , ADC_12bit,11);
headAD[2]=ad_ave (ADC0,AD13  , ADC_12bit,11);     
//headAD[0]=(int )((int)(headAD[0]/2)*2+OLD[0])/2;//滤波:先丢掉一个精度，然后关联历史参数
//headAD[1]=(int )((int)(headAD[1]/2)*2+OLD[1])/2;//滤波:先丢掉一个精度，然后关联历史参数
//OLD[0]=headAD[0];
//OLD[1]=headAD[1];
}
